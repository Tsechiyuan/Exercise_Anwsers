...
\begin{align}
  \Gamma_{ij}^k(p)=0
\end{align}
这是因为经过$p$的测地线由线性方程给出。

现在设$(u_i)$是$p\in U \subset M$周围的正规坐标系。设$(u_i,v_j)$是$TM$上的坐标。我们断言$TM$上的在$(q,v)$处的体积形式同构于是$U\times U$在$(q,q)$上的体积形式。

由于$G$是测地的,从而$G$是水平的向量场。从而我们可以在乘积度量下计算$\mathrm{div}G$:
\begin{align}
  G(u_i)=v_i G(v_j)=-\sum_{ik}\Gamma_{ik}^j v_iv_k
\end{align}

最终我们有:
\begin{align}
  \mathrm{div}G=\sum_i \frac{\partial v_i}{\partial u_i}-\sum_j \pa{v_j}(\sum_{ik}\Gamma_{ik}^j v_iv_k)=0
\end{align}
\section{第四章}
\end{document}
